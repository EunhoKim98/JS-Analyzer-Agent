...
export function detectSinks(code: string, file: string, rules: RuleCard[]): SinkRecord[] {
  const ast = parseSafe(code);
  if (!ast) return regexFallbackSinks(code, file, rules);

  const lines = code.split('\n');
  const sinkFlow = rules.filter((r) => r.detector === 'sink-flow');
  const protoCard = rules.find((r) => r.detector === 'proto-pollution');
  const pmCard = rules.find((r) => r.detector === 'postmessage');
...
// When AST parse fails entirely (exotic minified syntax), recover coverage with a
// textual sink scan. Lower precision — everything is sink_only (no taint).
function regexFallbackSinks(code: string, file: string, rules: RuleCard[]): SinkRecord[] {
  const lines = code.split('\n');
  const out: SinkRecord[] = [];
  const seen = new Set<string>();
  let c = 0;
  ...
  for (const card of rules.filter((r) => r.detector === 'sink-flow')) {
