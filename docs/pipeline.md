...
flowchart TD
    ...
    B["[1] unbundle<br/>소스맵 복원 / beautify + 페이지 JS"]
...
| # | 단계 | 종류 | 하는 일 |
|---|---|---|---|
| 0 | **ingest** | 결정론 | 파일·디렉터리·URL을 입력받아 content-hash로 정규화. 페이지 URL이면 다음 단계에서 스크립트를 수집. |
| 1 | **unbundle** | 결정론 | 소스맵이 있으면 원본 복원, 없으면 beautify. 헤드리스 크로미움(Playwright)으로 외부·인라인·동적 주입 스크립트까지 확보. |
