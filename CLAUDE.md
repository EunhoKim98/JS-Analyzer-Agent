...
```bash
npm install            # 의존성 설치 (playwright chromium은 최초 1회 npx playwright install chromium)
...
CLI(Cli/ArgParser)
  └─ runPipeline (합성 루트: 모든 협력자 생성·주입)
       └─ Pipeline.run(RunContext)
            [0] AcquireStage   ─ SourceAcquirer(Strategy) → FileIngestor | BrowserDiscoverer
...
| `src/ingest/browser.ts` | `BrowserDiscoverer` | 헤드리스 페이지 JS 수집 |
...
| `src/agents/client.ts` | `AuthResolver`, `LlmClient` | 인증 해석 + 엄격 JSON LLM 호출 (DIP) |
...
- **새 LLM 에이전트**: `LlmAgent`를 상속하고 `Agent<In,Out>`를 구현한다. `LlmClient`는 재사용한다.
