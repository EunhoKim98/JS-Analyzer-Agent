...
- **Claude Code / CLI**: **파일·디렉토리 또는 직접 `.js` URL**을 입력받아 원문을 그대로 수집한다.
  헤드리스 브라우저 로드·자동 크롤링·동적 스크립트 gap-fill은 하지 않는다(브라우저 동봉 불필요,
  바이너리 자기완결). 페이지 전체의 JS가 필요하면 Burp 확장 시드를
  쓴다(테스터가 실제 만진 트래픽만 분석).

수집된 각 JS는 content-hash로 dedup(`dedupeFiles`) 후 [1] 언번들로 진입.

> **개정(D7, 2026-07-21)**: 본래 v0.3 계획은 CLI가 헤드리스 브라우저로 페이지를 로드해 스크립트를
> 수집하는 것이었으나, Burp 철학(사용자 인터렉션 트래픽만)과 브라우저 패키징 문제(R2)로 인해
> **헤드리스 브라우저 경로를 폐기**하고 획득을 "파일/`.js` URL/Burp 시드"로 한정했다.
...
- **PoC 러너 3종**:
  - `browser`: 헤드리스 브라우저로 대상 페이지 로드 + URL/hash에 **양성 카나리
...
- **v0.6**: 활성 검증(§7.1) — 헤드리스 브라우저 PoC 러너(browser/http/url) + 아웃바운드 가드레일(§14.1). 기본 OFF.
